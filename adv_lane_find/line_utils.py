...
class Line:
    ...
    def __init__(self,buffer_len =10):
        # Flag to mark if line was detected the last iteration
        self.detected =False
        #  Polynomial coefficient fitted on the last iteration
        ...
        # Store all pixel coords (x,y) of line detected
        self.all_x = None
        self.all_y = None
    ...
    def draw(self,mask,color = (255,0,0), line_width = 50, average = False):
        ...
        # To recast the x and y points into usable format for cv2.fillPoly()
    ...
    @property
    # Average of polynomial coefficient of the last N iterations
...
def get_fits_by_sliding_windows(birdeye_binary,line_lt,line_rt,n_windows = 9,verbose=False):
    """ Get polymomial coefficients for lane-lines detected in a binary image """
    height,width = birdeye_binary.shape
    # Assuming you have created a warped binary image called "binary_waped"
    ...
    # Identify thhe x and y positions of all nonzero pixels in the image
    ...
    for window in range(n_windows):
        ...
        # If you found > minpix pixels, recenter next window onn their mean position
    ...
    # Concatinating the array indices
    left_lane_inds = np.concatenate(left_lane_inds)
    right_lane_inds = np.concatenate(right_lane_inds)

    # Extracting left and right pixel position
    ...
    # Generation of x,y values for plotting
...
def get_fits_by_previous_fits(birdeye_binary,line_lt,line_rt,verbose=False):
    """ For getting the polinomial coefficient of the lane line detected in an binary image"""
    ...
    # Create an image to draw on and an image to show the selected window
    ...
    # Generation of a polygon to illustrate the search window area
    ...
    #  Drawing the line onto the warped blank image:
...
def draw_back_onto_the_road(img_undistorted,Minv,line_lt,line_rt,keep_state):
    ...
    road_wrap = np.zeros_like(img_undistorted,dtype=np.uint8)
    pts_left = np.array([np.transpose(np.vstack([left_fitx,ploty]))])
    pts_right = np.array([np.flipud(np.transpose(np.vstack([right_fitx,ploty])))])
    pts = np.hstack((pts_left,pts_right))
    cv2.fillPoly(road_wrap,np.int_([pts]),(0,255,0))
    road_dewarped=cv2.warpPerspective(road_wrap,Minv,(width,height)) #Wrapping back to original image space

    blend_onto_road = cv2.addWeighted(img_undistorted,1.,road_dewarped,0.3,0)

    # Separately drawing solid lines to highlight them
    line_wrap = np.zeros_like(img_undistorted)
    line_wrap = line_lt.draw(line_wrap,color=(255,0,0),average = keep_state)
    line_wrap = line_rt.draw(line_wrap,color=(0,0,255),average = keep_state)
    line_dewarped = cv2.warpPerspective(line_wrap,Minv,(width,height))
